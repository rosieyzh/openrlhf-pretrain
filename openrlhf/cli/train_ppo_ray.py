...
if __name__ == "__main__":
    ...
    # PPO
    parser.add_argument("--clip", action="store_true", default=True, help="Enable policy gradient clipping (PPO-style)")
    parser.add_argument("--no_clip", action="store_false", dest="clip", help="Disable policy gradient clipping (for vanilla REINFORCE)")
    parser.add_argument("--importance_sampling", action="store_true", default=True, help="Use importance sampling ratio for policy updates")
    parser.add_argument("--no_importance_sampling", action="store_false", dest="importance_sampling", help="Directly use log probabilities instead of ratios")
    ...
    parser.add_argument("--eps_clip_low", type=float, default=None, help="Lower bound for asymmetric PPO clip range")
    parser.add_argument("--eps_clip_high", type=float, default=None, help="Upper bound for asymmetric PPO clip range")
    ...
    parser.add_argument(
        ...
        help="Type of optimizer to use (adam or ademamix)"
    )
    parser.add_argument("--adam_betas", type=float, nargs=2, default=(0.9, 0.95), help="Betas for Adam optimizer")
    parser.add_argument("--ademamix_betas", type=float, nargs=2, default=(0.98, 0.95), help="Betas for AdEMAMix optimizer")
    parser.add_argument("--ademamix_alpha", type=float, default=0.0, help="Alpha parameter for AdEMAMix optimizer")
    parser.add_argument("--ademamix_beta1_warmup", type=int, default=2000, help="Steps for beta1 warmup in AdEMAMix")
    parser.add_argument("--ademamix_min_beta1", type=float, default=0.9, help="Minimum beta1 value for AdEMAMix")
    parser.add_argument("--ademamix_bias_correction1", action="store_true", default=True, help="Use bias correction for first moment")
    parser.add_argument("--ademamix_bias_correction2", action="store_true", default=False, help="Use bias correction for second moment")
    ...
    if args.eps_clip_low is not None and args.eps_clip_high is not None:
        args.eps_clip = (args.eps_clip_low, args.eps_clip_high)
    # Alex: hardcoded
    today = datetime.today().strftime('%Y%m%d')
    args.wandb_run_name = f"{today}_{args.wandb_prefix}_{args.advantage_estimator}_{args.prompt_data.split('/')[-1]}_kl_{args.init_kl_coef}_temp_{args.temperature}_rollout_{args.rollout_batch_size}_nsamples_{args.n_samples_per_prompt}"
    ...
    if args.optimizer_type == "adam":
        args.ademamix_alpha = args.ademamix_beta1_warmup = args.ademamix_min_beta1 = args.ademamix_bias_correction1 = args.ademamix_bias_correction2 = None
        args.ademamix_weight_decay = args.l2
    elif args.optimizer_type == "ademamix":
        args.adam_betas = None
        args.adam_weight_decay = args.l2
