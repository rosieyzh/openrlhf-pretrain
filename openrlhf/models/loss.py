...
class PolicyLoss(nn.Module):
    ...
    def __init__(self, eps_clip: Union[float, tuple] = 0.2, clip: bool = True, importance_sampling: bool = True) -> None:
        super().__init__()

        if isinstance(eps_clip, tuple):
            self.eps_clip_low, self.eps_clip_high = eps_clip[0], eps_clip[1]
        else:
            self.eps_clip_low = -eps_clip
            self.eps_clip_high = eps_clip

        self.clip = clip 
        self.importance_sampling = importance_sampling
        self.clipping_fraction = 0.0
        self.clipping_avg_magnitude = 0.0
        self.clipping_max_magnitude = 0.0
    ...
    def forward(
        ...
        with torch.no_grad():
            if not self.clip:
                self.clipping_fraction = self.clipping_avg_magnitude = self.clipping_max_magnitude = 0.0
            else:
                clipped = (ratio < 1 + self.eps_clip_low) | (ratio > 1 + self.eps_clip_high)
                deviation_high = torch.where(ratio > 1 + self.eps_clip_high, ratio - (1 + self.eps_clip_high), torch.zeros_like(ratio))
                deviation_low = torch.where(ratio < 1 + self.eps_clip_low, (1 + self.eps_clip_low) - ratio, torch.zeros_like(ratio))
                deviation = deviation_high + deviation_low
                
                total = action_mask.sum().item() if action_mask is not None else clipped.numel()
                self.clipping_fraction = clipped.sum().item() / total if total > 0 else 0.0
                
                if clipped.sum() > 0:
                    masked_deviation = deviation * action_mask if action_mask is not None else deviation
                    self.clipping_avg_magnitude = masked_deviation.sum().item() / total
                    self.clipping_max_magnitude = masked_deviation.max().item()
                else:
                    self.clipping_avg_magnitude = self.clipping_max_magnitude = 0.0
                
        surr1 = ratio * advantages
        if self.clip:
            surr2 = ratio.clamp(1 + self.eps_clip_low, 1 + self.eps_clip_high) * advantages
        else:
            surr2 = ratio * advantages
        loss = -torch.min(surr1, surr2)
        loss = masked_mean(loss, action_mask, dim=-1).mean()
        return loss
        
    def get_clip_fraction(self) -> float:
        """Return the fraction of tokens that were clipped in the last forward pass."""
        return self.clipping_fraction
        
    def get_clip_avg_magnitude(self) -> float:
        """Return the average magnitude of deviation for clipped tokens."""
        return self.clipping_avg_magnitude
        
    def get_clip_max_magnitude(self) -> float:
        """Return the maximum magnitude of deviation for clipped tokens."""
        return self.clipping_max_magnitude
...
class ValueLoss(nn.Module):
    ...
    def __init__(self, eps_clip: float = None) -> None:
        super().__init__()
    ...
    def forward(
        ...
        if self.eps_clip is not None:
            values_clipped = old_values + (values - old_values).clamp(-self.eps_clip, self.eps_clip)
