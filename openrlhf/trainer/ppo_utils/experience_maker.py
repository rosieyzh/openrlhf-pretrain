...
class RemoteExperienceMaker(BaseExperienceMaker):
    ...
    def make_experience_list(
        ...
        eval_mode = generate_kwargs.get('eval', False)
    ...
    def compute_advantages_and_returns(
        ...
        if not eval_mode:
            ...
            # reward shaping
            rewards = torch.cat(rewards).reshape(-1).to(device="cpu").chunk(len(experiences))
    ...
    def _generate_vllm(self, all_prompts: List[str], all_labels, **kwargs) -> List[Samples]:
        ...
        eval_mode = kwargs.get('eval', False)
        print(f"Running in eval mode: {eval_mode}")
        ...
        if not eval_mode:
            ...
            all_labels = sum([[label] * 1 for label in all_labels], [])
