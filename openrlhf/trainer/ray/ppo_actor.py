...
from openrlhf.utils.optimizers import create_optimizer
...
class ActorPPOTrainer(BasePPOTrainer):
    ...
    def fit(
        ...
        if args.save_log_scale_count != -1:
            total_steps = args.num_episodes * prompts_dataloader.__len__()
            logspace = np.logspace(-2.1, 0, args.save_log_scale_count) * total_steps
            args.steps_to_save = [int(n) for n in logspace]
            print(f"Steps to save: {args.steps_to_save}")
        else:
            args.steps_to_save = None

        self.prompts_dataloader = prompts_dataloader
        self.pretrain_dataloader = pretrain_dataloader
        self.eval_dataloader = eval_dataloader

        if args.eval_steps != 0 and len(self.eval_dataloader) > 0:
            self.evaluate(self.eval_dataloader, 0)
        ...
        for episode in range(start_episode, args.num_episodes):
            ...
            for rand_prompts, labels in self.prompts_dataloader:
                ...
                pbar.update()
                steps = steps + 1
            # At end of episode, save checkpoint
            if args.ckpt_path is not None:
                tag = f"episode{episode}"
    ...
    def _log_clipping_heatmap(self, global_steps, clipping_frequencies):
        """Log cumulative clipping frequency heatmap to wandb."""

        if not hasattr(self, '_cumulative_clipping_heatmap_data'):
            self._cumulative_clipping_heatmap_data = []

        new_data = [[global_steps, i, cf] for i, cf in enumerate(clipping_frequencies)]
        self._cumulative_clipping_heatmap_data.extend(new_data)
        
        self._wandb.log({
            "train/clipping_heatmap": self._wandb.plot.scatter(
                self._wandb.Table(
                    data=self._cumulative_clipping_heatmap_data, 
                    columns=["Global Step", "Batch Index", "Clipping Frequency"]
                ),
                "Global Step", "Batch Index"
            ),
        })
    ...
    def ppo_train_actor(self, global_steps):
        ...
        for epoch in range(self.max_epochs):
            ...
            for experience in pbar:
                ...
                if "policy_loss" in status:
                    ...
                    # Include clipping metrics if available
                    if "clipping_fraction" in status:
                        short_status["clip"] = status["clipping_fraction"]
                        clipping_fractions.append(status["clipping_fraction"])
        ...
        # Log clipping fraction if we collected any and wandb is available
        if clipping_fractions and self._wandb is not None and self.strategy.is_rank_0():
            # Log heatmap
            self._log_clipping_heatmap(global_steps, clipping_fractions)
    ...
    def training_step(self, experience: Experience) -> Dict[str, float]:
        ...
        # with GatheredParameters(self.actor.model.parameters(), modifier_rank=0):
        #     grad_stats = self.compute_gradient_statistics(self.actor.model)
        # grad_stats = self.compute_gradient_statistics_distributed(self.actor.model)
        ...
        if hasattr(self.actor_loss_fn, 'get_clip_fraction'):
            status["clipping_fraction"] = self.actor_loss_fn.get_clip_fraction()
            status["clipping_avg_magnitude"] = self.actor_loss_fn.get_clip_avg_magnitude()
            status["clipping_max_magnitude"] = self.actor_loss_fn.get_clip_max_magnitude()

        # if self._wandb is not None and self.strategy.is_rank_0():
        #     for k, v in grad_stats.items():
        #         status[f"train/grad_{k}"] = v
    ...
    def evaluate(self, eval_dataloader, steps=0):
        ...
        self.actor.eval()
        self.eval_kwargs = copy.deepcopy(self.generate_kwargs)
        self.eval_kwargs['eval'] = True
        self.eval_kwargs['temperature'] = 0.0
        with torch.no_grad():
            step_bar = tqdm(
            ...
            acc_sum = 0
            rewards = []
            for rand_prompts, labels in self.eval_dataloader:
                # n_samples_per_prompt
                experiences = self.experience_maker.make_experience_list(rand_prompts, labels, **self.eval_kwargs)
                rewards += torch.cat([experience.info["reward"] for experience in experiences]).tolist()

        logs = {
            "acc_mean": np.mean(rewards),
        }
    ...
    def save_logs_and_checkpoints(self, args, global_step, step_bar, logs_dict={}, client_states={}):
        ...
        if global_step % args.save_steps == 0:
            ...
            self._save_checkpoint(args, tag, client_states)
        if args.steps_to_save and global_step in args.steps_to_save:
            tag = f"global_step{global_step}"
            self._save_checkpoint(args, tag, client_states)
    ...
    def _save_checkpoint(self, args, tag, client_states):
        ...
        if not self.disable_ds_ckpt:
            if self.critic_train_remote:
                ray.get(ref)
        torch.distributed.barrier()
    # def compute_gradient_statistics(self, model):
    #     """Compute basic gradient statistics efficiently across model parameters."""
        
    #     total_params = sum_grad = sum_sq_grad = 0.0
    #     max_grad = 0.0
    ...
    #             sum_grad += grad.sum().item()
    #             sum_sq_grad += (grad ** 2).sum().item()
    #             max_grad = max(max_grad, grad.abs().max().item())

    #     print(f"total_params: {total_params}, sum_grad: {sum_grad}, sum_sq_grad: {sum_sq_grad}, max_grad: {max_grad}")
    #     if total_params > 0:
    #         return {
    #             'mean': sum_grad / total_params,
    #             'variance': (sum_sq_grad / total_params) - (sum_grad / total_params) ** 2,
    #             'norm': np.sqrt(sum_sq_grad),
    #             'max': max_grad
    #         }
    #     return {}

    def compute_gradient_statistics_distributed(self, model):
        """Compute gradient statistics across distributed parameters"""
        stats = {
            'sum_grad': 0.0,
            'sum_sq_grad': 0.0,
            'max_grad': 0.0,
        ...
        # Compute local statistics on this GPU's parameter partitions
        for param in model.parameters():
            if param.grad is not None:
                grad = param.grad.detach()
                stats['total_params'] += grad.numel()
                stats['sum_grad'] += grad.sum().item()
                stats['sum_sq_grad'] += (grad ** 2).sum().item()
                stats['max_grad'] = max(stats['max_grad'], grad.abs().max().item())
        
        # Gather statistics across all processes
        all_stats = [torch.tensor([stats['total_params'], stats['sum_grad'], 
                                stats['sum_sq_grad'], stats['max_grad']]).to(torch.cuda.current_device())]
        dist.all_reduce_coalesced(all_stats, op=dist.ReduceOp.SUM)
        
        total_params, sum_grad, sum_sq_grad, max_grad = all_stats[0].tolist()
        print(f"total_params: {total_params}, sum_grad: {sum_grad}, sum_sq_grad: {sum_sq_grad}, max_grad: {max_grad}")
        if total_params > 0:
            return {
                'mean': sum_grad / total_params,
...
class ActorModelRayActor(BasePPORole):
    def init_model_from_pretrained(self, strategy: DeepspeedStrategy, pretrain):
        ...
        # configure optimizer
        actor_optim = create_optimizer(actor, strategy.args)
