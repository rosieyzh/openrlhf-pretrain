...
class BasePPOTrainer(ABC):
    ...
    Args:
        ...
        eps_clip (Union[float, tuple], defaults to 0.2): Clipping coefficient for policy loss.
        ...
        clip (bool): Whether to use clipping.
        importance_sampling (bool): Whether to use importance sampling.
    ...
    def __init__(
        ...
        eps_clip: Union[float, tuple] = 0.2,
        ...
        **generate_kwargs,
        ...
        clip = getattr(self.args, "clip", True)
        importance_sampling = getattr(self.args, "importance_sampling", True)
        if not clip: print("Clipping is disabled")
        if not importance_sampling: print("Disable importance sampling")

        self.actor_loss_fn = PolicyLoss(eps_clip, clip=clip, importance_sampling=importance_sampling)
