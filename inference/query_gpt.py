...
def compare_and_query(client, model, baseline_results, ckpt_results, save_dir):
    ...
    writer = jsonlines.Writer(open(os.path.join(save_dir, "judge.json"), 'w', encoding="utf-8"))
    for question in tqdm(questions):
        ...
        if baseline_q['acc'] == 0.0 and ckpt_q['acc'] == 1.0:
            ...
            ckpt_ans = ckpt_q['completions'][0]['output']
...
if __name__ == '__main__':
    ...
    baseline_results = load_file(os.path.join(args.base, 'eval_math_1.json'))
    ckpt_results = load_file(os.path.join(args.ckpt, 'eval_math_1.json'))
