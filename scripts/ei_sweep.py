...
def run(cli_args):
    ...
    for key in list(overrides.keys()):
        if key in base_sweep.keys() or key == 'pretrain' or key == 'generator': continue
        new_path += [key]
        new_path += [str(overrides[key])]
    
    if "generator" not in overrides.keys():
        overrides["generator"] = overrides["pretrain"]

    overrides['wandb_run_name'] = '_'.join(new_path)
    overrides['ei_iter_suffix'] = '_' + '_'.join(new_path)
    print(overrides)
    if "debug" in cli_args:
        print(len(config_list), config_list)

    launch_args = [
        "deepspeed --module openrlhf.cli.train_ei",
        f"--packing_samples",
        f"--bf16",
        f"--flash_attn",
